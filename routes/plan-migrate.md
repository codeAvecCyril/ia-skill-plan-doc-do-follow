...
3. **Remove `feat-status.md` files.** Feature status now lives in the `epic-status.md` Feature table; create or complete that table (from `template-epic-status.md`) where v2 left it missing. Carry over any blockers or notes worth keeping before deleting.
...
5. **Bootstrap the living docs** (skip those that do not apply):
   ...
   - `docs/design-guidelines.md`: point to the existing design system if the repo has one; otherwise write the minimal version from `template-design-guidelines.md`.
