...
1. Verify `docs/project-status.md` contains `E{n}`. If the epic is already 🟡 or later, warn and confirm re-specification
...
4. Ask questions on any ambiguity in requirements, scope, or priorities (Invariant 8)
5. Generate `epic-prd.md` from `templates/epic-prd.md`; write every requirement as a complete sentence a stakeholder can validate (Invariant 6)
6. Run the **prd-critic** subagent and handle its findings (SKILL.md → Subagents)
...
9. Run Status Sync (epic becomes 🟡). Handoff: `plan/epic-review E{n} prd`
