...
2. For each wave, run one **feature-coder** subagent (`subagents/feature-coder.md`) per task with a **task packet**: the full task text, its target file paths, the referenced pattern file content, the DONE criteria, and the repository instruction file content. Do **not** tell subagents to read the PRD, the architecture, or the whole tasks file — the packet is the context (this is what keeps token cost linear)
