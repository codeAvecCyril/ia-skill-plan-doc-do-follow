...
6. Run Status Sync. Changelog per the hard rule: only a feature-verified 🟢 one-liner in the epic changelog (and a project-changelog line only on epic completion) — wave progress, gate results, and task details never get entries. Handoff: `plan/feat E{n} F{n+1}`, `plan/epic E{n+1}` if the epic is done, or the gap list with the recommended fixing route
