...
4. **UI gate**: if the feature has UI requirements, its entry point exists in `docs/ui-map.md` and the **ui-consistency-reviewer** subagent (`subagents/ui-consistency-reviewer.md`) passes; apply its findings before approving.
5. **Performance gate (conditional)**: check the trigger criteria at the top of `subagents/perf-critic-backend.md` and `subagents/perf-critic-frontend.md` against the feature; for each side that fires, run that perf critic on the implemented hot paths and resolve its blocker findings before approving. If none fires, skip and note it.
...
3. If all gates pass: set the feature to 🟢 and run Status Sync. If the epic thereby completes, run the **doc-coherence-reviewer** subagent (`subagents/doc-coherence-reviewer.md`) and fix reported drift.
